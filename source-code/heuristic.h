...
void USWB::init(vector<int> &init_solution)
{
...
int USWB::nearestPowerOfTen(double num)
{
...
long long USWB::closestPowerOfTen(double num)
{
    if (num <= 5) return 1;
...
long long USWB::floorToPowerOfTen(double x)
{
...
void USWB::local_search_with_decimation(char *inputfile)
{
        ...
        for (step = 1; step < max_flips; ++step)
        {
            if (hard_unsat_nb == 0)
            {
                ...
                if (soft_unsat_weight < opt_unsat_weight)
                {
                    ...
                    cout << "o " << soft_unsat_weight << " " << total_step << " " << clause_weight[soft_clause_num_index[0]] << endl;
...
void USWB::hard_increase_weights()
{
    int i, c, v;
...
void USWB::soft_increase_weights_partial()
{
    int i, c, v;
...
void USWB::soft_increase_weights_not_partial()
{
    int i, c, v;
...
void USWB::update_clause_weights()
{
